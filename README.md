...
3. config에서 선택하면 다음 메시지부터 새 스타일이 적용됩니다. (Claude Code v2.1.251 이전 버전에서는 새 세션을 시작하거나 `/clear`를 사용해야 적용됩니다.) 다만 스타일 md 파일 자체를 새로 만들거나 수정한 경우에는, Claude Code가 시작할 때 파일을 읽으므로 재시작해야 반영됩니다.
